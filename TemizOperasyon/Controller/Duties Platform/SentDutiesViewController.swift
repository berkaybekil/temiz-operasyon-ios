//
//  SentDutiesViewController.swift
//  TemizOperasyon
//
...
class SentDutiesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func textFieldDidChange(_ textField: UITextField) {
        ...
        if textField.text == ""{
            self.sortedDutiesArary = self.dutiesArray
            self.tableView.reloadData()
        } else {
            
            self.sortedDutiesArary = [dutyModel]()
            
            for x in 0 ..< dutiesArray.count {
                
                let searchString: String = searchField.text!
                
                if dutiesArray[x].admin_user_name.lowercased().contains(searchString) || dutiesArray[x].barcode_number.lowercased().contains(searchString) || dutiesArray[x].subject.lowercased().contains(searchString) {
                    
                    self.sortedDutiesArary.append(self.dutiesArray[x])
        ...
        fetchData(endpoint: "task/list", parameters: parameters as [String : AnyObject]?) { response in
            ...
            if let json = response.object as? Dictionary<String, AnyObject> {
                ...
                if let data = json["data"] as? Dictionary<String, AnyObject> {
                    ...
                    if let tasks = data["task_from_me_data"] as? [Dictionary<String, AnyObject>] {
                        ...
                        for x in 0 ..< tasks.count {
                            ...
                            self.dutiesArray.append(item)
                            
                        }
                        
                        self.sortedDutiesArary = self.dutiesArray
                        self.pageTitleLbl.text = "Gönderdiklerim (\(self.dutiesArray.count))"
